...
# For BracketIRE+ Task
class TMRNetPLUSModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        return parser

    def __init__(self, opt):
        super(TMRNetPLUSModel, self).__init__(opt)
...
class TMRNet(nn.Module):
    def __init__(self, opt, mid_channels=64, max_residue_magnitude=10):
        ...
        for i in range(0,5):
            self.backbone['backward_rec_%d'%(i+1)] = ResidualBlocksWithInputConv1(3 * mid_channels, mid_channels)
...
class ResidualBlocksWithInputConv1(nn.Module):
