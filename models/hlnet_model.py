...
# For BracketIRE+ Task
class HLNetModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        return parser

    def __init__(self, opt):
        super(HLNetModel, self).__init__(opt)

        self.opt = opt
        self.loss_names = ['HLNet_l1', 'Total']
        self.visual_names = ['data_gt', 'data_in', 'data_out'] 
        self.model_names = ['HLNet']
        self.optimizer_names = ['HLNet_optimizer_%s' % opt.optimizer]

        tmrnet = HLNet(opt)
        self.netHLNet = N.init_net(tmrnet, opt.init_type, opt.init_gain, opt.gpu_ids)

        if self.isTrain:	
            self.optimizer_HLNet = optim.AdamW(self.netHLNet.parameters(),
                                              lr=opt.lr,
                                              betas=(opt.beta1, opt.beta2),
                                              weight_decay=opt.weight_decay)
            self.optimizers = [self.optimizer_HLNet]
    ...
    def forward(self):
        if self.isTrain:
            self.data_raws, self.data_gt = N.augment(self.data_raws, self.data_gt)
        ...
        if self.isTrain or (not self.isTrain and not self.opt.chop):
            self.data_out = self.netHLNet(self.data_raws)
        elif self.opt.chop:
        ...
        for i in range(num_h):
            for j in range(num_w):
                out[:, :, i*chop_size*s:(i+1)*chop_size*s, j*chop_size*s:(j+1)*chop_size*s] = self.netTMRNet(
                    new_input0.unsqueeze(0)[:,:,:,i*chop_size:(i+1)*chop_size, j*chop_size:(j+1)*chop_size])
    ...
    def backward(self, epoch):
        ...
        self.loss_Total = self.loss_TMRNet_l1
        self.loss_Total.backward()

    def optimize_parameters(self, epoch):
        self.forward()
        self.optimizer_HLNet.zero_grad()
        self.backward(epoch)
        self.optimizer_HLNet.step()
...
class HLNet(nn.Module):
    def __init__(self, opt, mid_channels=64, max_residue_magnitude=10):
        ...
        self.backbone['backward'] = ResidualBlocksWithInputConv2(3 * mid_channels, mid_channels, 24)

        for i in range(0,5):
            self.backbone['backward_rec_%d'%(i+1)] = HLFDB(3 * mid_channels, mid_channels)
    ...
    def upsample(self, lqs, feats, base_feat):
        ...
        hr_up = self.lrelu(self.conv_hr(hr_up))
        hr_up = self.conv_last(hr_up)
        return hr_up
    ...
    def forward(self, lqs):
        ...
        feats_ = self.feat_extract(lqs_in)   # (N*T, C, H, W)
        h, w = feats_.shape[2:]
        ...
        feats_ = torch.cat((ref_feat, oth_feat), dim=1)   # (N, T, C, H, W)
...
class ResidualBlocksWithInputConv2(nn.Module):
    def __init__(self, in_channels, out_channels=64, num_blocks=30):
        ...
        main.append(
            N.make_layer(
                ResidualBlockNoBN2, num_blocks, mid_channels=out_channels))
